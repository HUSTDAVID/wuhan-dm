    ...
    private final Handler handler=new Handler(){
        ...
        public void handleMessage(android.os.Message msg) {
    		if(msg.what==MSG_GET_FAV){
    			...
    			getFavTask.execute(countsPerPage,curPage);//param1:counts per page; param2:current page
    		}
        ...
        btnDel.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!WH_DMApp.isConnected){
					NotificationUtil.showShortToast(getString(R.string.check_network), CollectActivity.this);
					return;
				}
		        if(!WH_DMApp.isLogin){
		        	NotificationUtil.showShortToast(getString(R.string.please_login), CollectActivity.this);
					return ;
		        }
    ...
    private class GetFavTask extends AsyncTask<Integer,Void,ArrayList<Favorite>>{
		...
		protected void onPostExecute(ArrayList<Favorite> result){
			...
			if(result==null||result.size()==0){
				if(FLAG_PAGE_UP){
					 NotificationUtil.showShortToast(getString(R.string.no_more_message),CollectActivity.this);
					 FLAG_PAGE_UP=false;
			    }
				else{
					if(WH_DMApp.isConnected)
					{
						if(!WH_DMApp.isLogin)
						    ...
						    NotificationUtil.showShortToast(reason.toString(),CollectActivity.this);
					}
					else
						 NotificationUtil.showShortToast(getString(R.string.check_network),CollectActivity.this);
				}
